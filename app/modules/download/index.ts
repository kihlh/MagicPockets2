 ...
 */
import axios = require("superagent")
 ...
 * @returns 
 */
async function download(Url: string, Path?: string, referer?: string): Promise<Buffer | string> {
    Url = resolveUrl(Url);
    ...
    if (Path) {
        ...
        if (!fs.existsSync(Dir)) {
            if (Dir) await fs.mkdirs(Dir)
            // asg.jpg
            if (!Dir) Path = path.join(process.resourcesPath||__dirname, '..', "UserData", "temp", "cache")
        }
        let getData: fs.WriteStream;
        return new Promise(function (resolve, reject) {
            if(!Path)throw new Error("Not Path")
 ...
 * @param pthread 并发数
 */
async function downloadAll(UrlList: string | string[], Ext?: string, at?: string, pthread?: number) {
    if (!Ext) Ext = ""; if (!at) at =""; if (!pthread) pthread = 5;
    if (!Array.isArray(UrlList)) UrlList = [UrlList];
    let Concurrent:Promise<any>[] = [];
    let OKKConcurrent: PromiseSettledResult<Buffer | string>[] = []
    async function Gotu() {        
    ...
    for (const URL of UrlList) {
        ...
        if (Concurrent.length >= pthread) await Gotu()
    }
    // 漏网之鱼
    if (Concurrent.length) await Gotu()
    return OKKConcurrent
}
export { download, downloadAll,axios }