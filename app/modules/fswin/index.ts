...
class StructureFswin {
    ...
    */
    ...
    nsfw(Path: string,Options?: DirWatcherOptions){
     ...
     * @param Options 配置
     */
    dirWatcher(Path: string, callback: (Events: DirWatcherEventName, Message: DirWatcherMessage, QuickApi?: QuickApi) => void, Options?: DirWatcherOptions): dirWatcher {
        ...
        if (!Attributes.IS_DIRECTORY) {
            ...
            if (!_fswin.getAttributesSync(HigherPath)?.IS_DIRECTORY) {
        ...
        let StartFileWatcher = new _fswin.dirWatcher(Path, function (Events, Message) {
            ...
            if (Events == "ADDED" || Events == "MODIFIED") SetQuickApi = new QuickApi(String(Message));
            else if ((Events == "RENAMED" || Events == "MOVED") && typeof Message == "object") SetQuickApi = new QuickApi(Message.NEW_NAME);
            callback.apply(StartFileWatcher, [Events, Message, SetQuickApi])
        }, options);
...
class FileSystemQuickApi {
    ...
    constructor(QuickApi: QuickApi) {
        this.#QuickApi = QuickApi;
        this.#Path = QuickApi.path;
    }
    get size() {
        return this.Attributes?.SIZE || 0;
        // return fswin.sizeSync(this.#Path);
    }
    /**
      * 删除此文件
      * @param callback 
    ...
    remove(callback?: ((err: Error) => void) | undefined): void | Promise<void> {
        return fs.remove(this.#Path, callback || undefined);
    }
    /**
     * 移动该文件
     * @param newPath 新的位置 
     * @param callback 
     */
    move(newPath: string, callback?: ((err: Error) => void) | undefined) {
        if (path.parse(this.#Path).root == path.parse(newPath).root) {
            return callback ? fs.rename(this.#Path, newPath, callback) : fs.rename(this.#Path, newPath);
        }
        return callback ? fs.move(this.#Path, newPath, callback) : fs.rename(this.#Path, newPath);
    }
    /**
     * 是否存在该文件
     * @returns 
     ...
     */
    get isdir() {
        return this.Attributes?.IS_DIRECTORY || false;
    }
    /**是否是文件 */
    get isfile() {
        let { Attributes } = this;
        return Attributes && !Attributes.IS_DIRECTORY && !Attributes.IS_DEVICE;
    }
    #Att_50ms?: FileSystemWin32Apis.SystemFileAttributesW | null;

    get Attributes() {
        if (this.#Att_50ms) return this.#Att_50ms;
        this.#Att_50ms = fswin.getAttributesSync(this.#Path);
        setTimeout(() => this.#Att_50ms = undefined, 50);
        return this.#Att_50ms;
    }
...
class QuickApi {
    #Path: string = ""
    #FileSystemFast?: FileSystemQuickApi;
    #NotFile?: boolean;
    /**获取当前操作的路径 */
    get path() {
        return this.#Path;
    ...
    /**对比输入的路径和此文件路径是否一致 */
    diff(Path: string) {
        return path.resolve(Path) == path.resolve(this.#Path);
    }
    /**文件快速操作的API */
    get qfs() {
        if (this.#NotFile == false) return undefined;
        if (!this.#FileSystemFast) this.#FileSystemFast = new FileSystemQuickApi(this);
        return this.#FileSystemFast;
    }
    /**获取文件大小 */
    get size() {
        return fswin.sizeSync(this.#Path)
