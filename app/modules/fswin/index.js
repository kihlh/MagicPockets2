    ...
    dirWatcher(Path, callback, Options) {
        ...
        let StartFileWatcher = new _fswin.dirWatcher(Path, function (Events, Message) {
            ...
            if (Events == "ADDED" || Events == "MODIFIED")
                SetQuickApi = new QuickApi(String(Message));
            else if ((Events == "RENAMED" || Events == "MOVED") && typeof Message == "object")
                SetQuickApi = new QuickApi(Message.NEW_NAME);
...
class FileSystemQuickApi {
    ...
    /**
      ...
      */
    remove(callback) {
        return fs_extra_1.default.remove(this.#Path, callback || undefined);
    }
    /**
     * 移动该文件
     * @param newPath 新的位置
    ...
    move(newPath, callback) {
        if (path_1.default.parse(this.#Path).root == path_1.default.parse(newPath).root) {
            return callback ? fs_extra_1.default.rename(this.#Path, newPath, callback) : fs_extra_1.default.rename(this.#Path, newPath);
        }
        return callback ? fs_extra_1.default.move(this.#Path, newPath, callback) : fs_extra_1.default.rename(this.#Path, newPath);
    }
    /**
     * 是否存在该文件
     * @returns
...
class QuickApi {
    ...
    /**获取文件名 不包含后缀 */
    get name() {
        return path_1.default.parse(this.#Path).name;
    }
    /**获取文件全名 例: name.cpp */
    get base() {
    ...
    /**获取文件夹 */
    get dir() {
        return path_1.default.parse(this.#Path).dir;
    }
    /**获取后缀(包含.) 例: .jpg */
    get ext() {
    ...
    /**返回格式化后的文件 */
    get resolve() {
        return path_1.default.resolve(this.#Path);
    }
    /**对比输入的路径和此文件路径是否一致 */
    diff(Path) {
        return path_1.default.resolve(Path) == path_1.default.resolve(this.#Path);
    }
    /**文件快速操作的API */
    get qfs() {
        if (this.#NotFile == false)
            return undefined;
        if (!this.#FileSystemFast)
            this.#FileSystemFast = new FileSystemQuickApi(this);
