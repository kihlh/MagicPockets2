...
let fswin: FileSystemWin32Api | null =process.platform === "win32"? require(BinPath): null;
export { fswin };
if (!fswin) throw new Error("The current environment cannot complete the initialization");
export default fswin;
//用来声明类型（在napi里实现了） 没有实际用途
...
export declare namespace FileSystemWin32Apis {
    export class dirWatcher {
        ...
        constructor(
            dirToWatch: string,
            callback: (
                Events: DirWatcherEventName,
                Message: DirWatcherMessage
            ) => void | dirWatcherCallBackList,
    ...
    export function convertPath(
        ...
        CallBack: (Mess: ConvertResult | null) => void,
        Resolve: boolean
    ): void;
    /**
     ...
     * - 文件不存在：null
     */
    export function convertPathSync(
        pathToConvert: string,
        Resolve: boolean
    ): ConvertResult | null;
    /**
    ...
    export function find(
        pathToFind: string,
        Callback: (event: FindEvent, message: FindSystemFileAttributesW) => void,
        isProgressiveMode: true
    ): any;
    export function find(
        pathToFind: string,
        Callback: (message: FindSystemFileAttributesW[]) => void,
     ...
     * ```
     */
    export function findSync(pathToFind: string): string[];
    export function findSync(pathToFind: string, Callback: () => void): number;
     ...
     * @returns
     */
    export function getLogicalDriveList(
        CallBack: (Data: LogicalDriveList) => void
    ): void;
    /**
     * 获取所有磁盘号和设备类型  同步
     * @returns
     ...
     * @param CallBack  回调
     */
    export function getDriveDevice(
        id: EjectDrive_Letter,
        CallBack: (DriveStDevice: DriveStDevice | null) => void
    ): void;
    /**
     * 获取设备驱动ID 同步
     * @param id 盘号
     */
    export function getDriveDeviceSync(
        id: EjectDrive_Letter
    ): DriveStDevice | null;
    /**
     * 获取存储设备驱动信息 异步
    ...
    export function getDeviceCapabilities(
        id: GetStorageProperties_ID,
        Callback: (CM_DEVCAP: CM_DEVCAP | null) => void
    ): void;
    /**
     * 获取存储设备驱动信息 同步
     * @param id 盘号/设备ID
     */
    export function getDeviceCapabilitiesSync(
        id: GetStorageProperties_ID
    ): CM_DEVCAP | null;
    /**
     ...
     * @param Callback 回调
     */
    export function getStorageProperties(
        id: GetStorageProperties_ID,
        Callback: (CM_DEVCAP: StoragePropertiesData | null) => void
    ): void;
    /**
     * 通过驱动id获取驱动器的属性(超详细) 同步
     * @param id 盘号/设备ID
     */
    export function getStoragePropertiesSync(
        id: GetStorageProperties_ID
    ): StoragePropertiesData | null;
    /**
     *  获取磁盘基础信息 异步
     *  @param Path 根目录
     *  @param CallBack 回调
     */
    export function getVolumeInformation(
        Path: string,
        CallBack: (VolumeInformation: VolumeInformation | null) => void
    ): void;
    /**
     *获取磁盘基础信息 同步
     *  @param Path 根目录
     */
    export function getVolumeInformationSync(
        Path: string
     ...
     * @param CallBack  回调
     */
    export function getVolumeSpace(
        path: string,
        CallBack: (VolumeSpace: VolumeSpace | null) => void
    ): void;
    /**
     * 获取存储使用情况（字节）同步
     * @param path 根目录
     */
    export function getVolumeSpaceSync(path: string): VolumeSpace | null;
    /**
     * 设置卷名
     * @param path 路径
     * @param label 标记卷的字符串
     * @param callback 回调
     */
    export function setVolumeLabel(
        path: string,
        label: string,
     ...
     * @param label 标记卷的字符串
     */
    export function setVolumeLabelSync(path: string, label: string): boolean;
    /**
     * 获取原始文件特性常量  异步
     * @param Path 路径
     * @param CallBack  回调
    ...
    export function getAttributes(
        Path: string,
        CallBack: (Attributes: SystemFileAttributesW | null) => void
    ): void;
    /**
     * 获取原始文件特性常量  同步
     * @param Path 路径
     * @returns
     */
    export function getAttributesSync(Path: string): SystemFileAttributesW | null;
    /**
     *设置原始文件特性常量  异步
    ...
    export function setAttributes(
        ...
        attributes: SystemFileAttributesW,
        callback: (info: boolean) => void
    ): any;
    /**
     *设置原始文件特性常量  同步
     * @param pathToFileOrDir 路径
     ...
     * @returns
     */
    export function setAttributesSync(
        pathToFileOrDir: string,
        attributes: SystemFileAttributesW
    ): any;
    /**
     ...
     * - 2 使用程序调用弹出  调用的是：HotPlug.dll的HotPlugEjectDevice
     */
    export function ejectDriveSync(
        letter: EjectDrive_Letter,
        method: 0 | 1 | 2
    ): any;
    /**
    ...
    export const ntfs: {
        ...
        getCompressedSize(
            Path: string,
            CallBack: (CompressedSizeInfo: number) => void
        ): void;
        /**
         * 获取NTFS压缩文件后的在磁盘大小 如果不存在或者没有此文件将返回0  如果一致将返回相同的值
         * @param Path 路径
     ...
     * 磁盘驱动名称
     */
    export type GetStorageProperties_ID =
        | "\\\\?\\PhysicalDrive0"
        | "\\\\?\\PhysicalDrive1"
        | "\\\\?\\PhysicalDrive2"
        | "\\\\?\\PhysicalDrive3"
     ...
     * - "FIXED"  磁盘
     */
    export type LogicalDriveList = {
        [K in Uppercase<EjectDrive_Letter>]: "FIXED" | "REMOVABLE";
    }[];

    /**盘号 */
    export type EjectDrive_Letter =
        | "A"
        | "B"
        | "C"
        | "D"
     ...
     * - ENDED 结束观察器
     */
    export type DirWatcherEventName =
        | "ERROR"
        | "RENAMED"
        | "STARTED"
        | "MOVED"
     ...
     *监听器消息类型
     */
    export type DirWatcherMessage =
        | string
        | DirWatcherRenamedMessage
        | DirWatcherErrorMessage;
    /**
     * 错误类型
     ...
     */
    export type DirWatcherErrorMessage =
        | "UNABLE_TO_CONTINUE_WATCHING"
        | "UNABLE_TO_WATCH_SELF"
        | "INITIALIZATION_FAILED";

    /**只有更新文件夹才会使用该方法 */
    export type DirWatcherRenamedMessage = { NEW_NAME: string; OLD_NAME: string };
     ...
     * 存储属性数据
     */
    export type StoragePropertiesData = {
        /**访问对齐属性 */
     ...
     */
    export type SystemFileAttributesW = {
        /**创建时间*/
     ...
     * https://docs.microsoft.com/en-us/windows/win32/fileio/file-attribute-constants
     */
    export type FindSystemFileAttributesW = {
        /**文件名(含后缀) */
        LONG_NAME: string;
        /**简称 例如：C:\\PROGRA~1 */
        SHORT_NAME: string;
     ...
     * - INTERRUPTED 中断
     */
    export type FindEvent = "FOUND" | "SUCCEEDED" | "FAILED" | "INTERRUPTED";
    /**格式化短链接返回 */
    export type ConvertResult = null | string;

    export type dirWatcherCallBackList =
        | ((event: "ERROR", message: DirWatcherErrorMessage) => void)
        | ((event: "RENAMED", message: DirWatcherRenamedMessage) => void)
        | ((event: "ENDED", message: string) => void)
        | ((event: "MODIFIED", message: string) => void)
        | ((event: "REMOVED", message: string) => void)
        | ((event: "ADDED", message: string) => void)
        | ((event: "MOVED", message: DirWatcherRenamedMessage) => void)
        | ((event: "STARTED", message: string) => void);
}
