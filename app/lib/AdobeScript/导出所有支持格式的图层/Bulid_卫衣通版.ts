...
let { fsName: ActiveDocumentFilePath } = app?.activeDocument?.path;
if (!ActiveDocumentFilePath) alert("当前未打开文档");
let ActiveDocumentName = app.activeDocument.name;
let { displayName: ActiveDocumentDirName } = app.activeDocument.path.displayName;
let DefaultDir = "~/Pictures/Saved Pictures/kiic";
let ___Dir = DefaultDir;
let is_build_To = ActiveDocumentName.match(/build[.]psd/i) ? true : false;
let is_Join_Entrance = null;
let ID = "K9V7O9NLGG3GP";
let ColorList = ["黑色", "深棕", "灰色", "白色", "深蓝", "浅灰", "藕粉", "薄荷色", "抹茶绿", "木炭灰", "驼色", "杏色", "咖啡", "卡其", "棕色", "黄色", "浅蓝色", "绿色", "克莱因蓝"];
let PushBlackPrintsList = ["白色", "黄色", "杏色", "驼色", "抹茶绿", "浅灰"];
/**
...
*运行脚本
*/
function evalJavaScript(jsxPath: string) {
...
function exportMainImages() {
    ...
    let is_Drew = ActiveDocumentName.match(/!--单面_DrewT恤带阴影800|!--单面_T恤带阴影800/);
    let is_Drew_1200 = ActiveDocumentName.match(/!--单面_DrewT恤带阴影1200|!--单面_T恤带阴影1200/);

    let is_Prints = (is_Drew || is_Drew_1200 || is_build_To) ? false : confirm("Print？");
    let is_ToPNG = (is_Drew || is_Drew_1200 || is_build_To) ? false : confirm("ToPNG?");
    ...
    function HideLayer(Name?: string) {
        if (Name) {
    ...
    function ArrayHas(ArrayData: any[], data: any) {
        for (let data_ of ArrayData) {
            if (data == data_) return true;
        }
        return false
    }
    ...
    // 获取所有图层
    let AllLayers: Layer[] = [];
    let Get_layers: Layer[] = app.activeDocument.layers;

    /**
     * 遍历此文档所有图层
     * @param Get_layers 
     */
    function ForEach_Layers(layersMainList: Layer[]) {
        for (const layers of layersMainList) {
    ...
    function export_All(dirs) {
        ...
        for (let Layers of AllLayers) {
            let { name: Lname } = Layers;
            let envName = Lname.replace(/背面|正面|前面|后面|背部/, '');
            if (!is_Join_Entrance && Lname.match(/抽绳/)) continue;
            if (ArrayHas(ColorList, envName)) {
                ...
                if (is_Prints) {
                    ...
                    // 是否是黑色印花
                    let isBlack = ArrayHas(PushBlackPrintsList, envName);
                ...
                // 不添加吊牌的规则
                if (is_Drew || is_Drew_1200) {
                    if (Lname.match(/背部|后面|背面/) || ActiveDocumentFilePath.match(/2296|2299/) || is_ToPNG) HideLayer("LEE实物吊牌");
                    else ShowLayer("LEE实物吊牌");

                }

                if (!is_ToPNG) lookNameToSaveJPEG(Lname, dirs);
                else {
    ...
    if (is_Drew) {
        let ClassifyFormat = ActiveDocumentName.match(/Drew/) ? "Drew" : "King";

        let env_ = is_ToPNG ? true : false;
        app.doAction("Dr_invoke_导出主图", "Build");
        is_ToPNG = false;
        let Path_0 = DefaultDir + ClassifyFormat + "\\800";
        ...
        $.sleep(150);
        let Path_1 = DefaultDir + ClassifyFormat + "\\SKU";
        console.log("Path_1=>"+Path_1)
        export_All(Path_1);
        is_ToPNG = true;
        app.doAction("Dr_invoke_导出透明", "Build");
        $.sleep(150);
        let Path_2 = DefaultDir + ClassifyFormat + "\\透明";
        ...
        let Path_1 = DefaultDir + ClassifyFormat + "\\1200";
        is_ToPNG = false;
        export_All(Path_1);
    ...
    if (is_Prints) HideAllFlowers();
    function GetRandomly() {
        if (Class_AllLayers.length) {
            for (var i = 0; i < 1000; i++) {
                let For_INDEX = Math.floor(Math.random() * 10);
    ...
    if (Class_AllLayers.length) ShowLayer(GetRandomly());

    declare var charIDToTypeID: any, executeAction: any, stringIDToTypeID: any
...
if (ActiveDocumentName.match(/build[.]psd/i)) {
    let List_ = ["800", "1200", "!--单面_DrewT恤带阴影800", "!--单面_DrewT恤带阴影1200", "!--单面_T恤带阴影800", "!--单面_T恤带阴影1200"];
    LayerEdit("详情");
    $.sleep(1000);
    app.doAction("导出详情", "Build");
    app.activeDocument.close(SaveOptions.DONOTSAVECHANGES);

    for (const __name of List_) {
        LayerEdit(__name);
        $.sleep(1000);
        exportMainImages()
        app.activeDocument.close(SaveOptions.DONOTSAVECHANGES);
