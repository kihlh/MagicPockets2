...
var is_Prints = is_Drew || confirm("Print？");
var is_ToPNG = is_Drew || confirm("ToPNG?");
var is_Transfers = null;
var ___Dir = "~/Pictures/Saved Pictures/kiic";

if (is_Drew) {
  is_Prints = false;

  if (app.activeDocument.name.match(/!--单面_DrewT恤带阴影800/)) {
    ___Dir += "/Drew";
  } else {
    ___Dir += "/KING";
  }
...
mkFolder(___Dir); // if (typeof is_Prints == 'object' && is_Prints == null) is_Prints = confirm("Print？");
...
var PushBlackPrintsList = ["白色", "黄色", "杏色", "驼色", "抹茶绿", "浅灰"]; // 选择黑色印花
...
function evalJavaScript(jsxPath) {
  ...
  var idjsMs = charIDToTypeID("jsMs"); // desc1786.putString( idjsMs, "","undefined","" );
...
function export_All(dirs) {
  for (var _i = 0, AllLayers_2 = AllLayers; _i < AllLayers_2.length; _i++) {
    ...
    var envName = Lname.replace(/背面|正面|前面|后面|背部/, ''); // alert(Lname)
    ...
    if (ArrayHas(ColorList, envName)) {
      ...
      if (!is_ToPNG) lookNameToSaveJPEG(Lname, dirs);else {
...
for (var _i = 0, AllLayers_1 = AllLayers; _i < AllLayers_1.length; _i++) {
  ...
  var Lname = Layers_2.name,
      L_kind = Layers_2.kind;
  var envName = Lname.replace(/背面|正面|前面|后面|背部/, '');

  if (ArrayHas(ColorList, envName)) {
    Class_AllLayers.push(Layers_2);
    HideLayer(Lname); // alert(Lname)
  }
...
// 隐藏所有匹配的图层
for (let Layers of app.activeDocument.layers) {
...
for (let Layers of app.activeDocument.layers) {
    ...
    if (ArrayHas(ColorList, envName)) {
        ...
        if (!is_ToPNG) lookNameToSaveJPEG(Lname, null);
        else {
            lookNameToSavePNG(Lname, null);
        }
    }
