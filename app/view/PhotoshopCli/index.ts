...
import "../../modules/tdesign/tdesign.css";
import "../../modules/tdesign/tdesign.js";
/// <reference types="node" />
import _Vue, { RenderFunction } from 'vue';
// import Vue,{ createApp } from 'vue';
import _TDesign from 'tdesign-vue-next';
import robotjs from "../../modules/_NapiModules/robotjs";
import open from "open";
// import { Sleep } from "../../modules/_NapiModules/win32api";
import ImageMagick from "../../modules/Magick-Cli/index";
...
import "../../lib/css.gg/css/bolt.css"
import "../../lib/css.gg/css/erase.css"
import "../../lib/css.gg/css/ericsson.css"
import "../../lib/css.gg/css/add.css"
import "../../lib/css.gg/css/backspace.css"
import { GenerateForDrew } from "./plugins/GenerateForDrew";
...
Photoshop_Path_List.push(...HMC.getProcessNameFilePath("Photoshop.exe"));
...
function App_Main_VM_Data(this: any) {
    return {
        ...
        TopBtn: [
            {
                ...
                click: () => {
                    ...
                    if (app.SelectMenuFunctions && app.MainFunctionsList) {
                        ...
                        let DataList: Set<any> = new Set(app.MainFunctionsList);
        ...
        SelectMenuFunctions: null as null | { path: string, name: string },
        visible: false,
        Top_prompt_model: "",
    }
}
let appMainData = window.Vue.defineComponent({
    ...
    methods: {
        openCodeEditor(Itme: { path: string, Select?: boolean, name: string, icon: string, FunctionList: {}[], click?: (Itme: any) => void }) {
         ...
         * @returns 
         */
        MainFunctionsChange(index: number) {
         ...
         * @param Itme 
         */
        ClickMainFunctions(Itme: { Select?: boolean, name: string, icon: string, FunctionList: {}[], click?: (Itme: any) => void }) {
            if (Itme.name == "便捷") {
        ...
        contextmenuMenuFunctionsValue(Itme: any) {
         ...
         * @param Itme 点击了选择了方法
         */
        SelectMenuFunctionsValue(Itme: any) {
...
// @ts-expect-error
const app: vm = window.Vue.createApp(appMainData).use(window.TDesign).mount("#app");
app.$nextTick().then(() => {
...
function SelectPrint(Path: string) {
...
async function NotificationPhotoshop(Path: string | string[], Notclear?: boolean) {
    ...
    for (const Paths of (Path && Array.isArray(Path) ? Path : [Path]) || [""]) {
        ...
        if (Paths.includes("选择黑色") || Paths.includes("选择白色")) {
            if (Paths.includes("选择黑色")) await SelectPrint(pathSelectBlackPrint);
            if (Paths.includes("选择白色")) await SelectPrint(pathSelectWhitePrint);
        }
...
document.addEventListener("keydown", (Events) => {
    ...
    function moveElement(arr: any[], n: number) {
...
function clipPathMirrorToSave() {
    ...
    for (let Path of FileList) ForImagesFileList.push({ path: path.resolve(Path), dir: path.dirname(Path) });
...
async function Clip_PSD_To(SetExt?: ".png" | ".jpg" | ".jpeg" | "png" | "jpeg" | "jpg", ProcessLimit?: number) {
    const { cmd } = window;
    ...
    */
    ...
    let GotuSpawnList: _cmd.StartSpawn[] = [];
    // 剪贴板没有内容
    if (!GetClipDataType.length) throw new Error("Not Clip Data");
    // 复制的是文件列表
        ...
        if (!GetClipDataType.includes('image/png')) {
            ...
            for (const Paths of ClipFilePaths) {
                ...
                if (Stats.isDirectory()) {
                    window.walk.walkStream(Paths).on("data", function (chunk: _walk_.Entry) {
...
declare var window: Window & typeof globalThis & SetGlobalWindowListType & {
    Vue: typeof _Vue,
    TDesign: typeof _TDesign
} & {
    robotjs: typeof robotjs
    app: vm;
    clipPathMirrorToSave: typeof clipPathMirrorToSave
    GenerateForDrew: typeof GenerateForDrew
    Clip_PSD_To: typeof Clip_PSD_To
    Magick: ImageMagick;
    NotificationPhotoshop: typeof NotificationPhotoshop
    Clip_To_Link:typeof Clip_To_Link
}
